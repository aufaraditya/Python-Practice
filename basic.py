...
'''
--------------------------------------------- Challenge: Unique Number in List ------------------------------------

numbers = [2,3,4,43,6,4,22,1,1,1,22]
unique = []
for number in numbers:
...
'''
--------------------------------------------- Tutorial 1: Dictionary Mapping  ------------------------------------

numbers_in = input('Enter number to convert into words: ')
dictionary = {
          '0': 'Zero',
          '1': 'One',
          '2': 'Two',
...
for number in numbers_in:
    numbers_out += dictionary.get(number, 'NaN!') + ' ' #Get the mapping key

print(f"Your numbers are: {numbers_out}")
...
class Person:
    ...
    def talk(self): #self must be the first parameter
        print(f'Yo, I am {self.name}') #self.name is assigned above
